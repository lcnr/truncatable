...
#define umulrem(z, x, y, m)	\
	{	\
	z = (unsigned int)(x * (unsigned long long)y % m);	\
	}

#define umuladdrem(z, x, y, a, m)	\
	{	\
	z = (unsigned int)((x * (unsigned long long)y + a) % m);	\
	}
...
static unsigned int modpow(unsigned int n, unsigned int e, unsigned int m)
{
	unsigned int r = 1;
	unsigned int t = n % m;
	unsigned int i;
...
bool aks(unsigned long long n) {
	...
	for (r = 3; r < n; r += 2)
	{
		if (IsPrime(r))
		{
			...
			if (modpow(m, (r - 1) / q, r) <= 1) continue;
/*
			// AKS
